...
impl Interpreter {
 ...
 fn eval(&mut self,x:&Expr)->Result<Value,RuntimeError>{match x{
  ...
  Expr::Array(xs,_)=>Ok(Value::Array(xs.iter().map(|x|self.eval(x)).collect::<Result<Vec<_>,_>>()?)),
  Expr::Map(xs,_)=>{let mut m=HashMap::new();for(k,v) in xs{m.insert(k.clone(),self.eval(v)?);}Ok(Value::Map(m))},
  Expr::Index{target,index,..}=>{let collection=self.eval(target)?;let key=self.eval(index)?;match(collection,key){(Value::Array(xs),Value::Number(n))=>xs.get(n as usize).cloned().ok_or_else(||RuntimeError("array index out of bounds".into())),(Value::Map(m),Value::String(k))=>m.get(&k).cloned().ok_or_else(||RuntimeError(format!("missing map key `{k}`"))),_=>Err(RuntimeError("indexing requires an array and number or map and string".into()))}},
  Expr::Call{name,arguments,..}=>{if name=="length"{if arguments.len()!=1{return Err(RuntimeError("length expects one argument".into()));}return match self.eval(&arguments[0])?{Value::Array(xs)=>Ok(Value::Number(xs.len() as f64)),Value::Map(m)=>Ok(Value::Number(m.len() as f64)),Value::String(s)=>Ok(Value::Number(s.chars().count() as f64)),_=>Err(RuntimeError("length requires a string, array, or map".into()))};}let(params,body)=self.functions.get(name).cloned().ok_or_else(||RuntimeError(format!("unknown function `{name}`")))?;let args=arguments.iter().map(|a|self.eval(a)).collect::<Result<Vec<_>,_>>()?;self.push();for(p,v)in params.into_iter().zip(args){self.define(p,v);}let r=self.exec(&Stmt::Block(body))?;self.pop();Ok(match r{Flow::Continue=>Value::Null,Flow::Return(v)=>v})},
...
#[cfg(test)]mod collection_tests{use super::*;use crate::{lexer::Lexer,parser::Parser};#[test]fn executes_collection_program(){let src="begin\nlet xs be [1,2,3]\nprint xs[1]\nprint length(xs)\nend";let ast=Parser::new(Lexer::new(src).tokenize().unwrap()).parse_program().unwrap();assert!(Interpreter::new().execute(&ast).is_ok());}}
